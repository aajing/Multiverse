...
parser.add_argument("--direct_scene_feat", action="store_true")

# --- next -x
parser.add_argument("--grid_strides", default="2,4")

parser.add_argument("--add_grid", action="store_true")
parser.add_argument("--add_all_reg", action="store_true",
                    help="save all regression target in H,W")
...
def prepro_each(traj_path, split, prepro_path, args):
  ...
  if not videos:
    print("Warning, no video for split %s, skipped" % split)
    return False
  ...
  if args.add_grid:
    seq_grid_target_all_list = []  # [strides][N, seq_len, h, w, 2]
    for i, _ in enumerate(args.scene_grids):
      seq_grid_target_all_list.append([])
  ...
  for video in tqdm(videos, ascii=True):
    ...
    if args.add_scene:
      if args.direct_scene_feat:
        # we can access scene feature given frameIdx
        pass
      else:
        # get the frameid to file name since scene is not extracted every frames
        scene_file = os.path.join(args.scene_map_path, split,
                                  "%s.p" % videoname)
    ...
    if not data:
      print("warning, %s/%s is empty, skipped." % (split, videoname))
      continue
    ...
    for idx, frame in enumerate(frames):
      ...
      if args.add_grid:
        ...
        if args.add_all_reg:
          ...
          for i, (h, w) in enumerate(args.scene_grids):
            target = np.zeros(
      ...
      if args.add_scene:
        ...
        for i, frame_idx in enumerate(frame_idxs):
          ...
          if args.direct_scene_feat:
            key = os.path.join(args.scene_feat_path, videoname,
                               "%s_F_%08d.npy" % (videoname, frame_idx))
          else:
            # so we only load unique feat once
            # file path of the scene feature
  ...
  if args.add_grid:
    ...
    for i, center in enumerate(args.scene_grid_centers):
      ...
      if args.add_all_reg:
        # [N, T, h, w, 2]
        seq_grid_target_all_list[i] = np.concatenate(
            seq_grid_target_all_list[i], axis=0)
        obs_seq_grid_target_all = \
          seq_grid_target_all_list[i][:, :obs_len, :, :, :]
        pred_seq_grid_target_all = \
          seq_grid_target_all_list[i][:, obs_len:, :, :, :]
        data.update({
            ("obs_grid_target_all_%d" % i): obs_seq_grid_target_all,
